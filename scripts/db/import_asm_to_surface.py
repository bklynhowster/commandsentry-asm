...
# BOTH. So with that secret set on Prodex, Prodex real-time notification email
# goes out FROM a Command address, branded COMMANDsentry, linking to Command's
# portal — leaking Prodex fleet activity into Command's identity.
#
# Deliberately NO fallback to Command's values: a default is what lets a
# forgotten env leak silently. Unset => skip the fan-out, matching the existing
# graceful no-op when SENDGRID_API_KEY is missing. Losing a notification is
# strictly better than sending one as the wrong tenant.
...
def dispatch_event_notifications(
    ...
    # No cross-tenant fallback (see SENDGRID_FROM_EMAIL above): unset means we
    # do not know who we are, and sending as the wrong tenant is worse than not
    # sending. Same graceful-skip shape as the missing-API-key path above.
    # 4.7 M5 — assert transport config here, at the top of the fan-out, before
    # any per-recipient work. Differentiated message names the exact missing var.
    _mailer.warn_if_unconfigured(os.environ.get("INSTANCE_NAME", ""))
    ...
    for bucket_id, sub_list in buckets.items():
        ...
        with conn.cursor() as cur:
            ...
            if disc == "confirmed_live":
                ...
                # U6 — resurrection. Runs on the SAME condition as the clock bump above
                # (observed confirmed_live ⇒ service_count > 0), and is a no-op for every
                # asset that was not dark. Placed AFTER the clock so a resurrected asset
                # already has a fresh last_alive_at when its status flips.
                #
                # ⛔ Pass print here. Passing the bare name `l`+`og` (the shape this
                # shipped as) raises NameError: this module has no such function — it
                # prints, 23 call sites. NOTE: the offending token is split above on
                # purpose. Spelling it out made a CI guard match this COMMENT as if it
                # were the defect (4.8, relay 162), and made my own grep of the FIXED
                # file report 2 hits. Prose about a pattern satisfies a pin looking for
                # that pattern.
                # It raised at the CALL site for the FIRST confirmed_live asset of every
                # run, so the whole import failed:
                # Prodex ASM Discover #317 "import error: name 'log' is not defined ...
                ...
                # writer were SKIPPED — i.e. the dark gate flipped live hours earlier
                # silently stopped running. The no-op-for-non-dark-assets guard is INSIDE
