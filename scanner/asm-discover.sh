...
# v3 asset record.
#
# 2026-07-06: added subfinder `-all` + keyed sources (chaos/virustotal via the
# workflow's provider-config, from GH secrets) + source 5 (direct crt.sh). Fixes
# the datacenter-IP enumeration blind spot — the runner was silently missing subs
# a residential run finds because the free sources rate-limit datacenter IPs.
discover_apex() {
  local apex="$1" wd="$2"

  # ─── Source 1: passive (subfinder, ALL sources) ────────────
  # `-all` pulls every subfinder source, incl. keyed APIs (chaos/virustotal) via
  # the provider-config the asm-discover workflow writes from GH secrets before
  # this runs. Default-sources-only + a datacenter runner IP is what silently
  # missed subs (2026-07-06 enumeration-gap fix — free sources rate-limit
  # datacenter IPs; keyed sources meter by key, not IP). timeout 120 caps a
  # hung/slow source; partial output is still captured, ||warn keeps it non-fatal.
  ...
  # certs that haven't propagated to subfinder's aggregators yet. BELT-AND-
  # SUSPENDERS ONLY — crt.sh rate-limits and can time out (esp. from a datacenter
  # runner IP), so one shot with a timeout, tolerate empty, never load-bearing.
  # Same filtering as the other sources: lowercase, strip wildcards, within apex.
...
discover_ip() {
  ...
  if grep -qE '"port":(80|443|8080|8443)' "$wd/naabu.json" 2>/dev/null; then
    ...
    if grep -q '"port":443' "$wd/naabu.json" 2>/dev/null; then
      ...
      # false-positive factory: of 27 fleet cert_trust findings in the 07-21+
      # surge, 21 were artifacts of THIS path (7 bare IPs + 14 FQDNs that share
      # those IPs yet serve a perfectly valid cert to a real SNI client); only 6
      # were genuine name mismatches — and every one of those is independently
      # caught by the FQDN path (see the testssl call ~line 296, real SNI). So
      # drop ONLY the cert-hostname-match records from the IP-path JSON; all other
      # TLS checks (ciphers, protocols, chain-of-trust) are valid against an IP
      # and are kept. Belt-and-suspenders: match the exact testssl id AND the
      # mismatch text, so a testssl build that suffixes the id (e.g.
      # "cert_trust <cert#1>") is still caught, without touching cert_trust_wildcard
      # or cert_chain_of_trust.
